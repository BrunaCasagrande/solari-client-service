...
class ClientControllerTest {
  ...
  @Mock private CreateClient createClient;

  @Mock private DeleteClient deleteClient;

  @Mock private SearchClient searchClient;

  @Mock private UpdateClient updateClient;

  @Mock private ClientPresenter clientPresenter;
  ...
  void shouldCreateClient() throws Exception {
    ...
    ClientPresenterResponse response =
        new ClientPresenterResponse(
            1,
            "Bruna Casagrande",
            "12345678900",
            "bruna@example.com",
  ...
  void shouldFindClientByCpf() throws Exception {
    ...
    // Act & Assert
    mockMvc.perform(get("/solari/v1/clients/{cpf}", cpf)).andExpect(status().isOk());
  }
  ...
  void shouldReturnNotFoundWhenClientDoesNotExist() throws Exception {
    ...
    // Act & Assert
    mockMvc.perform(get("/solari/v1/clients/{cpf}", cpf)).andExpect(status().isNotFound());
  }
  ...
  void shouldDeleteClient() throws Exception {
    ...
    // Act & Assert
    mockMvc.perform(delete("/solari/v1/clients/{cpf}", cpf)).andExpect(status().isNoContent());
  }
}
